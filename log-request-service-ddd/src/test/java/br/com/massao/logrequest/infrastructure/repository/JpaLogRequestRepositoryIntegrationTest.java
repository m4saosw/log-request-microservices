...
//    JpaLogRequestRepository repository;
    ...
    void givenLogsWhenListThenReturnLogs() {
        ...
        List<DomainLogRequest> domains = Arrays.asList(domain1, domain2, domain3);
        Page<DomainLogRequest> pageDomain = new PageImpl<>(domains);

        // when
        Page<DomainLogRequest> results = repository.listAll(pageDomain.getPageable());

        // then
        assertThat(results.getContent().get(0).getRequest()).isEqualTo(domain1.getRequest());
        assertThat(results.getContent().get(1).getRequest()).isEqualTo(domain2.getRequest());
        assertThat(results.getContent().get(2).getRequest()).isEqualTo(domain3.getRequest());
    }
}